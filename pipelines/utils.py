...
# Actual token usage reported by the Gemini API for the LAST generate call
# (the brief requires actual API usage whenever available; tiktoken remains the
# cross-pipeline comparison basis and is disclosed as an estimate). Read this
# right after gemini_generate() returns.
LAST_USAGE: dict = {}
    ...
    for attempt in range(max_attempts):
        try:
            ...
            try:
                ...
                LAST_USAGE = {
                    "api_prompt_tokens": u.prompt_token_count,
                    "api_output_tokens": u.candidates_token_count,
                    "api_total_tokens": u.total_token_count,
                }
            except Exception:
                LAST_USAGE = {}
