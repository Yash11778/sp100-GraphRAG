...
def main(paths: list[str]):
    ...
    for p in paths:
        ...
        for name, sub in df.groupby("pipeline"):
            stats[name] = {
                ...
                "graded": sub["graded_score"].dropna().mean() if "graded_score" in sub else float("nan"),
                "citation": sub["citation_score"].dropna().mean() if "citation_score" in sub else float("nan"),
                "bert_f1": sub["bertscore_f1"].dropna().mean() if "bertscore_f1" in sub else float("nan"),
            }
    ...
    for n in pipelines:
        ...
        grd = [s[n]["graded"] for _, s in per_run if n in s]
        cit = [s[n]["citation"] for _, s in per_run if n in s]
        brt = [s[n]["bert_f1"] for _, s in per_run if n in s]
        if vals:
            print(f"  {n}: pass {sum(vals)/len(vals):.1f}%  "
                  f"(range {min(vals):.1f}-{max(vals):.1f})  "
                  f"graded {sum(grd)/len(grd):.2f}/3  cit {sum(cit)/len(cit):.2f}/2  "
                  f"bertF1 {sum(brt)/len(brt):.3f}  "
