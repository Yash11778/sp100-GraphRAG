...
1. **High Accuracy** — 56% pass vs 18% RAG (3.2×, leak-adjusted; 62% full-set)
2. **Token-Efficient** — ~35% fewer tokens than RAG (45% full-set)
...
- **03 — Impact & Results** *(green)*
  > **56% accuracy at ~35% fewer tokens (3.2× RAG)** — our leak-adjusted claim;
  > 62%/−45% on the full set. Ablation proves the graph is essential (−40 pts &
  > +49% tokens without it). Fully auditable, 0-token ingestion; a blind
  > entity-agnostic rule reproduces the graph edges, and an independent judge
  > agrees on 97.3% of verdicts.
