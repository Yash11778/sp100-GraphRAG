...
- **⚠ Adjusted (39 Qs, excluding 11 answer-key-informed): 56.4% vs 17.9% — 3.2× at
  −34.6% tokens.** This is the generalization claim; see the integrity slide.
...
router and traversal layer plus the two baselines. We **tune parameters only on a
small 18-question dev set and freeze the configuration before touching the held-out
questions**. One caveat we disclose rather than gloss: that freeze covered the
retriever, not the *ingestion vocabulary*, and our entity/topic extraction was
picked with the held-out keys visible — affecting 11 of 50 questions, which is why
we report an adjusted 39-question figure alongside the headline. Then come three
independent held-out runs plus ablations, and finally the live demo and
documentation. Each phase ends in a concrete, checkable milestone.
